...
namespace EventManagementSystem.Application.Utility
{
    public static class SD
    {
         ...
         List<Booking> bookings)
        {
            ...
            var roomsInVilla = villaNumberList.Where(x => x.EventId == villaId).Count();

            for (int i = 0; i < nights; i++)
            {
                var villasBooked = bookings.Where(u => u.CheckInDate <= checkInDate.AddDays(i)
                && u.CheckOutDate > checkInDate.AddDays(i) && u.EventId == villaId);
