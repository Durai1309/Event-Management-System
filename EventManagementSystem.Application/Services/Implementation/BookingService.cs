    ...
    public class BookingService : IBookingService
    {
        ...
        public IEnumerable<Booking> GetAllBookings(string userId = "", string? statusFilterList = "")
        {
            ...
            if (!string.IsNullOrEmpty(statusFilterList) && !string.IsNullOrEmpty(userId))
            {
                ...
                u.UserId == userId, includeProperties: "User,Event");
            }
            else
            {
                if (!string.IsNullOrEmpty(statusFilterList))
                {
                    return _unitOfWork.Booking.GetAll(u => statusList.Contains(u.Status.ToLower()), includeProperties: "User,Event");
                }
                if (!string.IsNullOrEmpty(userId))
                {
                    return _unitOfWork.Booking.GetAll(u => u.UserId == userId, includeProperties: "User,Event");
                }
            }
            return _unitOfWork.Booking.GetAll(includeProperties: "User,Event");
        }

        public Booking GetBookingById(int bookingId)
        {
            return _unitOfWork.Booking.Get(u => u.Id == bookingId, includeProperties: "User, Event");
        }

        public IEnumerable<int> GetCheckedInVillaNumbers(int eventId)
        {
            return _unitOfWork.Booking.GetAll(u => u.EventId == eventId && u.Status == SD.StatusCheckedIn)
                .Select(u => u.EventNumber);
        }

        public void UpdateStatus(int bookingId, string bookingStatus, int eventNumber = 0)
        {
            ...
            if (bookingFromDb != null)
            {
                ...
                {
                    bookingFromDb.EventNumber = eventNumber;
